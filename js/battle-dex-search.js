...
var searchIndex=BattleSearchIndex;
var searchIndexOffset=BattleSearchIndexOffset;
var closest=DexSearch.getClosest;
if(this.dex.modid.includes('digimon')){
searchIndex=DigimonSearchIndex;
searchIndexOffset=DigimonSearchIndexOffset;
closest=function(query){

var left=0;
var right=searchIndex.length-1;
while(right>left){
...
if(left>=searchIndex.length-1)left=searchIndex.length-1;else
if(searchIndex[left+1][0]&&searchIndex[left][0]<query)left++;
...
_id=searchIndex[originalIndex][0];
}else{
...
function BattleTypedSearch(searchType){var format=arguments.length>1&&arguments[1]!==undefined?arguments[1]:'';var speciesOrSet=arguments.length>2&&arguments[2]!==undefined?arguments[2]:'';this.searchType=void 0;this.dex=Dex;this.formats=[''];this.species='';this.set=null;this.baseResults=null;this.baseIllegalResults=null;this.illegalReasons=null;this.results=null;this.sortRow=null;
...
filters.length;_i10++){var _ref4=filters[_i10];var filterType=_ref4[0];var value=_ref4[1];
switch(filterType){
case'type':

if(!species.types.includes(value))return false;
break;
case'egggroup':
...
return results.sort(function(_ref5,_ref6){var rowType1=_ref5[0],id1=_ref5[1];var rowType2=_ref6[0],id2=_ref6[1];
...
return results.sort(function(_ref7,_ref8){var rowType1=_ref7[0],id1=_ref7[1];var rowType2=_ref8[0],id2=_ref8[1];
...
var skipped=['noability','mountaineer','rebound','persistent','allseeingeye'];
if(skipped.includes(_id8))continue;
...
if(this.formats.includes('gen9morebalancedhackmons')){
return[['header',"MBHv4"],['ability','allseeingeye'],['header',"Abilities"]].
concat(this.getDefaultResults());
}
if(
this.formats.includes('almostanyability')||
this.formats.includes('createmons')||
this.formats.includes('hackmons')||
this.formats.includes('metronome'))
return[['header',"Abilities"]].concat(this.getDefaultResults());
var dex=this.dex;
var species=dex.species.getFromPokemon(this.set);
var speciesAbilities=Object.assign({},species.abilities);
...
var ability=this.dex.abilities.get(row[1]);for(var _i12=0;_i12<
filters.length;_i12++){var _ref11=filters[_i12];var filterType=_ref11[0];var value=_ref11[1];
switch(filterType){
case'pokemon':
...
var speciesSpecific=[];for(var _i14=0;_i14<
results.length;_i14++){var _this$dex$items$get$i;var row=results[_i14];
...
filters.length;_i16++){var _ref12=filters[_i16];var filterType=_ref12[0];var value=_ref12[1];
switch(filterType){
case'pokemon':
if(!Dex.hasAbility(this.dex.species.get(value),ability.name))return false;
break;
...
var skipped=['paleowave','shadowstrike'];
if(skipped.includes(_id9))continue;
...
if(this.dex.modid.includes('stadium')){
...
if(this.dex.modid.includes('gen7letsgo')){
if(['megadrain','teleport'].includes(id))return true;
}

if(this.dex.modid.includes('metronome')){
...
return species.baseSpecies==='Keldeo'||this.dex.modid.includes('doubles');
case'infestation':
...
case'phantomforce':
return!moves.includes('poltergeist')&&!moves.includes('shadowclaw')||this.dex.modid.includes('doubles');
case'poisonfang':
...
return!moves.includes('earthquake')&&!moves.includes('drillrun')||this.dex.modid.includes('doubles');
case'stunspore':
...
if(this.dex.modid.includes('doubles')&&BattleMoveSearch.GOOD_DOUBLES_MOVES.includes(id)){
return true;
}
...
var moves=dex.getLearnsetMoves(this.set);
moves.sort();

var usableMoves=[];
var uselessMoves=[];for(var _i18=0;_i18<
moves.length;_i18++){var _id10=moves[_i18];
var isUsable=this.moveIsNotUseless(_id10,species,moves,this.set);
if(isUsable){
...
var move=this.dex.moves.get(row[1]);for(var _i20=0;_i20<
filters.length;_i20++){var _ref13=filters[_i20];var filterType=_ref13[0];var value=_ref13[1];
switch(filterType){
case'type':
