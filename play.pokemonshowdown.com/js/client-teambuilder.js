(function (exports, $) {
	...
	exports.TeambuilderRoom = exports.Room.extend({
		...
		initialize: function () {
			...
			if (typeof Storage.prefs("uploadprivacy") !== "undefined") {
				this.$("input[name=teamprivacy]").is(
					"checked",
					Storage.prefs("uploadprivacy")
				);
			}
		},
		focus: function () {
		...
		update: function () {
			...
			if (this.curTeam) {
				if (
					this.curTeam.format &&
					!this.formatResources[this.curTeam.format]
				) {
					this.tryLoadFormatResource(this.curTeam.format);
				}
				if (
					this.curTeam.loaded === false ||
					(this.curTeam.teamid && !this.curTeam.loaded)
				) {
					this.loadTeam();
				...
				this.ignoreEVLimits =
					this.curTeam.gen < 3 ||
					(this.curTeam.dex.modid.includes("hackmons") &&
						this.curTeam.gen !== 6) ||
					this.curTeam.dex.modid.includes("metronome");
				if (this.curSet) {
		...
		tryLoadFormatResource: function (format) {
			var teambuilder = this;
			if (format in teambuilder.formatResources) {
				// already loading, bypass
				return;
			}
			teambuilder.formatResources[format] = true; // true - loading, array - loaded
			$.get(
				"https://www.smogon.com/dex/api/formats/by-ps-name/" + format,
				{},
				function (data) {
		...
		updateTeamInterface: function () {
			...
			if (this.exportMode) {
				if (this.curFolder) {
					buf =
						'<div class="pad"><button name="back" class="button"><i class="fa fa-chevron-left"></i> List</button></div>';
					buf +=
						'<div class="teamedit"><textarea readonly class="textbox" rows="17">' +
						BattleLog.escapeHTML(Storage.exportFolder(this.curFolder)) +
						"</textarea></div>";
				} else {
					buf =
						'<div class="pad"><button name="back" class="button"><i class="fa fa-chevron-left"></i> List</button> <button name="saveBackup" class="savebutton button"><i class="fa fa-floppy-o"></i> Save</button></div>';
					buf +=
						'<div class="teamedit"><textarea class="textbox" rows="17">';
			...
			if (!Storage.whenTeamsLoaded.isLoaded) {
				if (Storage.whenTeamsLoaded.error === "stalled") {
					buf =
						'<div class="pad"><p class="message-error">We\'re having some trouble loading teams securely.</p>';
					buf +=
						"<p>This is sometimes caused by antiviruses like Avast and BitDefender.</p>";
					buf +=
						"<p><strong>If you're using Firefox and an antivirus:</strong> Your antivirus is trying to scan your teams, and a recent Firefox update doesn't let it. Turn off HTTPS scanning in your antivirus or uninstall your antivirus, and your teams will come back.</p>";
					buf +=
						"<p>You can use the teambuilder insecurely, but any teams you've saved securely won't be there.</p>";
					buf +=
						'<p><button class="button" name="insecureUse">Use teambuilder insecurely</button></p></div>';
				} else if (Storage.whenTeamsLoaded.error) {
					buf =
						'<div class="pad"><p class="message-error">We got an error trying to load teams: ' +
						Storage.whenTeamsLoaded.error.message +
						".</p>";
					buf +=
						"<p>This might be because you didn't give us permission to load teams: on macOS, this is in System Preferences → Security &amp; Privacy → Privacy → Files and Folders → Pokemon Showdown</p></div>";
				} else {
					buf =
						'<div class="pad"><p>lol zarel this is a horrible teambuilder</p>';
					buf +=
						"<p>that's because we're not done loading it...</p></div>";
				}
				this.$el.html(buf);
				return;
			}
		...
		updateFolderList: function () {
			var buf =
				'<div class="folderlist"><div class="folderlistbefore"></div>';

			buf +=
				'<div class="folder' +
				(!this.curFolder
					? ' cur"><div class="folderhack3"><div class="folderhack1"></div><div class="folderhack2"></div>'
					: '">') +
				'<div class="selectFolder" data-value="all"><em>(all)</em></div></div>' +
				(!this.curFolder ? "</div>" : "");
			var folderTable = {};
			var folders = [];
			if (Storage.teams)
				for (var i = -2; i < Storage.teams.length; i++) {
					if (i >= 0) {
					...
					switch (format.slice(0, 4)) {
						case "gen1":
							format = "I" + format.slice(4);
							break;
						case "gen2":
							format = "H" + format.slice(4);
							break;
						case "gen3":
							format = "G" + format.slice(4);
							break;
						case "gen4":
							format = "F" + format.slice(4);
							break;
						case "gen5":
							format = "E" + format.slice(4);
							break;
						case "gen6":
							format = "D" + format.slice(4);
							break;
						case "gen7":
							format = "C" + format.slice(4);
							break;
						case "gen8":
							format = "B" + format.slice(4);
							break;
						case "gen9":
							format = "A" + format.slice(4);
							break;
						default:
							format = "X" + format;
							break;
			...
			formatFolderBuf +=
				'<div class="folder"><div class="selectFolder" data-value="+"><i class="fa fa-plus"></i><em>(add format folder)</em></div></div>';
				...
				if (gen === "/") {
					...
					buf +=
						...
						(this.curFolder === format
							? ' cur"><div class="folderhack3"><div class="folderhack1"></div><div class="folderhack2"></div>'
							: '">') +
						'<div class="selectFolder" data-value="' +
						format +
						'"><i class="fa ' +
						(this.curFolder === format ? "fa-folder-open" : "fa-folder") +
						(format === "/" ? "-o" : "") +
						'"></i>' +
						formatName +
						"</div></div>" +
						(this.curFolder === format ? "</div>" : "");
					continue;
				}
				formatName = format.slice(1);
			...
			buf += '<div class="foldersep"></div>';
			buf +=
				'<div class="folder"><div class="selectFolder" data-value="++"><i class="fa fa-plus"></i><em>(add folder)</em></div></div>';
		...
		updateTeamList: function (resetScroll) {
			...
			if (!this.curFolder) {
				...
				buf +=
					'<h2>All teams <small style="font-weight: normal">(' +
					teams.length +
					")</small></h2>";
			} else {
				if (this.curFolder.slice(-1) === "/") {
					...
					if (filterFolder) {
						buf +=
							...
							filterFolder +
							' <button class="button small" style="margin-left:5px" name="renameFolder"><i class="fa fa-pencil"></i> Rename</button> <button class="button small" style="margin-left:5px" name="promptDeleteFolder"><i class="fa fa-times"></i> Remove</button></h2>';
					} else {
						buf +=
			...
			if (filterFormat && filterFormat !== "gen9") {
				newTeamButtonText =
					"New " + BattleLog.escapeFormat(filterFormat) + " Team";
			}
			buf +=
				'<p><button name="newTop" value="team" class="button big"><i class="fa fa-plus-circle"></i> ' +
				newTeamButtonText +
				"</button> " +
				'<button name="newTop" value="box" class="button big"><i class="fa fa-archive"></i> New Box</button> ' +
				'<input type="text" id="teamSearchBar" name="search" class="textbox searchinput" value="' +
				this.curSearchVal +
				'" placeholder="search teams"/></p>';

			buf += '<ul class="teamlist">';
			var atLeastOne = false;

			try {
				if (!window.localStorage && !window.nodewebkit)
					buf +=
						"<li>== CAN'T SAVE ==<br /><small>Your browser doesn't support <code>localStorage</code> and can't save teams! Update to a newer browser.</small></li>";
			} catch (e) {
				buf +=
					"<li>== CAN'T SAVE ==<br /><small><code>Cookies</code> are disabled so you can't save teams! Enable them in your browser settings.</small></li>";
			}
			if (Storage.cantSave)
				buf +=
					"<li>== CAN'T SAVE ==<br /><small>You hit your browser's limit for team storage! Please backup them and delete some of them. Your teams won't be saved until you're under the limit again.</small></li>";
			if (!teams.length) {
				if (this.deletedTeamLoc >= 0) {
					buf +=
						'<li><button name="undoDelete"><i class="fa fa-undo"></i> Undo Delete</button></li>';
				}
				...
				for (var i = 0; i < teams.length + 1; i++) {
					...
					if (!team) {
						...
						teams.splice(i, 1);
						i--;
						if (this.deletedTeamLoc && this.deletedTeamLoc > i)
							this.deletedTeamLoc--;
						continue;
					}

					if (filterFormat && filterFormat !== (team.format || "gen9"))
						continue;
					if (filterFolder !== undefined && filterFolder !== team.folder)
						continue;
					...
					if (this.curSearchVal) {
						...
						var searchVal = this.curSearchVal
							.split(",")
							.map(function (el) {
								return toID(el);
							});
					...
					buf +=
						...
						"<strong>" +
						BattleLog.escapeHTML(team.name) +
						"</strong><br /><small>";
					buf += Storage.getTeamIcons(team);
					buf +=
						'</small></div><button name="edit" value="' +
						i +
						'"><i class="fa fa-pencil" aria-label="Edit" title="Edit (you can also just click on the team)"></i></button><button name="duplicate" value="' +
						i +
						'" title="Duplicate" aria-label="Duplicate"><i class="fa fa-clone"></i></button><button name="delete" value="' +
						i +
				...
				if (!atLeastOne) {
					if (filterFolder) {
						buf +=
							"<li><p><em>you don't have any teams in this folder lol</em></p></li>";
					} else {
						buf +=
							"<li><p><em>you don't have any " +
							this.curFolder +
							" teams lol</em></p></li>";
					}
				}
			...
			if (atLeastOne) {
				buf +=
					'<button name="new" value="team" class="button"><i class="fa fa-plus-circle"></i> ' +
					newTeamButtonText +
					'</button> <button name="new" value="box" class="button"><i class="fa fa-archive"></i> New Box</button> ';
			}
			buf +=
				'<button class="button" name="send" value="/teams">View teams uploaded to server</button>';
			buf += "</p>";

			if (window.nodewebkit) {
				buf +=
					'<button name="revealFolder" class="button"><i class="fa fa-folder-open"></i> Reveal teams folder</button> <button name="reloadTeamsFolder" class="button"><i class="fa fa-refresh"></i> Reload teams files</button> <button name="backup" class="button"><i class="fa fa-upload"></i> Backup/Restore all teams</button>';
			} else if (this.curFolder) {
				buf +=
					'<button name="backup" class="button"><i class="fa fa-upload"></i> Backup all teams from this folder</button>';
			} else if (atLeastOne) {
				buf +=
					"<p><strong>Clearing your cookies (specifically, <code>localStorage</code>) will delete your teams.</strong> ";
				buf +=
					'<span class="storage-warning">Browsers sometimes randomly clear cookies - you should upload your teams to the Showdown database ';
				buf +=
					"or make a backup yourself if you want to make sure you don't lose them.</span></p>";
				buf +=
					'<button name="backup" class="button"><i class="fa fa-upload"></i> Backup/Restore all teams</button>';
				buf +=
					"<p>If you want to clear your cookies or <code>localStorage</code>, you can use the Backup/Restore feature to save your teams as text first.</p>";
				var self = this;
				...
				buf +=
					'<button name="backup" class="button"><i class="fa fa-upload"></i> Restore teams from backup</button>';
			}
		...
		greeting: function (answer, button) {
			...
			if (answer === "N") {
				buf +=
					"<p>Aww, that's too bad. :( I hope playing on Pok&eacute;mon Showdown today can help cheer you up!</p>";
			} else if (answer === "Y") {
				buf +=
					"<p>Cool! I just added some pretty cool teambuilder features, so I'm pretty happy, too. Did you know you can drag and drop teams to different format-folders? You can also drag and drop them to and from your computer (works best in Chrome).</p>";
				buf +=
					'<p><button class="button" name="greeting" value="W"><i class="fa fa-question-circle"></i> Wait, who are you? Talking to a teambuilder is weird.</button></p>';
			} else if (answer === "W") {
				buf += "<p>Oh, I'm Zarel! I made a Credits button for this...</p>";
				buf +=
					'<div class="menugroup"><p><a href="//pokemonshowdown.com/credits" target="_blank"><button class="button mainmenu4"><i class="fa fa-info-circle"></i> Credits</button></a></p></div>';
				buf +=
					"<p>Isn't it pretty? Matches your background and everything. It used to be in the Main Menu but we had to get rid of it to save space.</p>";
				buf +=
					'<p>Speaking of, you should try <button class="button" name="background"><i class="fa fa-picture-o"></i> changing your background</button>.';
				buf +=
					'<p><button class="button" name="greeting" value="B"><i class="fa fa-hand-pointer-o"></i> You might be having too much fun with these buttons and icons</button></p>';
			} else if (answer === "B") {
				buf +=
					"<p>I paid good money for those icons! I need to get my money's worth!</p>";
				buf +=
					'<p><button class="button" name="greeting" value="WR"><i class="fa fa-exclamation-triangle"></i> Wait, really?</button></p>';
			} else if (answer === "WR") {
				buf +=
					"<p>No, they were free. That just makes it easier to get my money's worth. Let's play rock paper scissors!</p>";
				buf +=
				...
				buf +=
					'<p>I play laser, I win. <i class="fa fa-hand-o-left"></i></p>';
				buf +=
					'<p><button class="button" name="greeting" value="YC"><i class="fa fa-thumbs-o-down"></i> You can\'t do that!</button></p>';
			} else if (answer === "SP") {
				buf +=
					"<p>Okay, sure. I warn you, I'm using the same RNG that makes Stone Edge miss for you.</p>";
				buf +=
					'<p><button class="button" name="greeting" value="SP3"><i class="fa fa-caret-square-o-right"></i> I want to play Rock Paper Scissors</button> <button class="button" name="greeting" value="SP5"><i class="fa fa-caret-square-o-right"></i> I want to play Rock Paper Scissors Lizard Spock</button></p>';
			} else if (answer === "SP3") {
				buf +=
					'<p><button class="button" name="greeting" value="PR3"><i class="fa fa-hand-rock-o"></i> Rock</button> <button class="button" name="greeting" value="PP3"><i class="fa fa-hand-paper-o"></i> Paper</button> <button class="button" name="greeting" value="PS3"><i class="fa fa-hand-scissors-o"></i> Scissors</button></p>';
			} else if (answer === "SP5") {
				buf +=
				...
				var my = ["R", "P", "S", "L", "K"][
					Math.floor(Math.random() * Number(answer[2]))
				];
				var your = answer[1];
				buf +=
					'<p>I play <i class="fa fa-hand-' +
					rpsChart[my] +
					'-o"></i> ' +
					rpsChart[my] +
					"!</p>";
				...
				if (!this.rpsScores || !this.rpsScores.length) {
					this.rpsScores = [
						...
						"28°C",
						"百万点",
						'<i class="fa fa-bitcoin"></i>0.0000174',
						'<s>priceless</s> <i class="fa fa-cc-mastercard"></i> MasterCard',
						"127.0.0.1",
						"C&minus;, see me after class",
					];
				}
				var score = this.rpsScores.splice(
					Math.floor(Math.random() * this.rpsScores.length),
					1
				)[0];
				buf += "<p>Score: " + score + "</p>";
				buf +=
					'<p><button class="button" name="greeting" value="SP' +
					answer[2] +
					'"><i class="fa fa-caret-square-o-right"></i> I demand a rematch!</button></p>';
			} else if (answer === "YC") {
				buf +=
					'<p>Okay, then I play peace sign <i class="fa fa-hand-peace-o"></i>, everyone signs a peace treaty, ending the war and ushering in a new era of prosperity.</p>';
				buf +=
					'<p><button class="button" name="greeting" value="SP"><i class="fa fa-caret-square-o-right"></i> I wanted to play for real...</button></p>';
			}
					...
					app.addPopup(PromptPopup, {
						...
						callback: function (name) {
							...
							if (name.indexOf("/") >= 0 || name.indexOf("\\") >= 0) {
								app.addPopupMessage(
									"Names can't contain slashes, since they're used as a folder separator."
								);
								name = name.replace(/[\\\/]/g, "");
							}
							if (name.indexOf("|") >= 0) {
								app.addPopupMessage(
									"Names can't contain the character |, since they're used for storing teams."
								);
								name = name.replace(/\|/g, "");
							}
							if (!name) return;
		...
		promptDeleteFolder: function () {
			app.addPopup(DeleteFolderPopup, {
		...
		edit: function (i) {
			...
			this.curTeam.dex = Dex.mod(this.curTeam.format);
			this.curTeam.gen = this.curTeam.dex.gen;
			Storage.activeSetList = this.curSetList = Storage.unpackTeam(
				this.curTeam.team
			);
			this.curTeamIndex = i;
			this.update();
		...
		createTeam: function (orig, isBox) {
			...
			if (
				...
				!/ OPR\/4[0-5]/.test(navigator.userAgent)
			) {
				var self = this;
		...
		psExport: function () {
			...
			var team = Storage.exportTeam(
				this.curSetList,
				this.curTeam.gen,
				false
			);
			if (!team)
				return app.addPopupMessage(
					"Add a Pokémon to your team before uploading it!"
				);
			buf.push(team);
		...
		pokepasteExport: function (type) {
			...
			if (this.curTeam.format !== "gen9") {
				document.getElementById("pasteNotes").value =
		...
		mouseOverTeam: function (e) {
			if (!e.currentTarget.className.endsWith("team-hover"))
				e.currentTarget.className += " team-hover";
		},
		mouseOutTeam: function (e) {
			if (e.currentTarget.className.endsWith("team-hover"))
				e.currentTarget.className = e.currentTarget.className.slice(0, -11);
		},
		dragStartTeam: function (e) {
			...
			dataTransfer.setData(
				"text/plain",
				"Team " + e.currentTarget.dataset.value
			);
			...
			var downloadurl =
				...
				urlprefix +
				encodeURIComponent(
					window.btoa(unescape(encodeURIComponent(contents)))
				);
		...
		finishDrop: function () {
			...
			if (newLoc !== originalLoc) {
				...
				for (var room in app.rooms) {
					...
					if (originalLoc === obj.curTeamIndex) {
						...
						originalLoc > obj.curTeamIndex &&
						newLoc <= obj.curTeamIndex
					) {
						obj.curTeamIndex++;
					} else if (
						originalLoc < obj.curTeamIndex &&
			...
			if (app.draggingFolder) {
				...
				if (!$plusOneFolder.length) {
					$folder.prepend(
						'<strong style="float:right;margin-right:3px;padding:0 2px;border-radius:3px;background:#CC8500;color:white" class="plusonefolder">+1</strong>'
					);
				} else {
			...
			if (this.finalOffset) {
				...
				$newTeamEl.css(
					"transform",
					"translate(" +
						(this.finalOffset[0] - finalPos.left) +
						"px, " +
						(this.finalOffset[1] - finalPos.top) +
						"px)"
				);
				setTimeout(function () {
		...
		dragEnterFolder: function (e) {
			...
			if (
				parseInt(app.dragging.dataset.value, 10) >= Storage.teams.length &&
				format.slice(-1) !== "/"
			) {
		...
		defaultDragEnterTeam: function (e) {
			...
			if (
				...
				dataTransfer.files[0].name.slice(-4) !== ".txt"
			)
				return;
			...
			this.$(".teamlist").append(
				'<li class="dragging"><div class="team" data-value="' +
					Storage.teams.length +
					'"></div></li>'
			);
		...
		defaultDropTeam: function (e) {
			if (
				...
				if (name.slice(-4) !== ".txt") {
					...
					this.updateTeamList();
					app.addPopupMessage(
						"Your file is not a valid team. Team files are .txt files."
					);
					return;
				}
			...
			this.finalOffset = [
				e.originalEvent.pageX - app.draggingOffsetX,
				e.originalEvent.pageY - app.draggingOffsetY,
			];
		},
		...
		updateTeamView: function () {
			...
			if (this.exportMode) {
				buf =
					'<div class="pad"><button name="back" class="button"><i class="fa fa-chevron-left"></i> List</button> <input class="textbox teamnameedit" type="text" class="teamnameedit" size="30" value="' +
					BattleLog.escapeHTML(this.curTeam.name) +
					'" /> <button name="saveImport" class="button"><i class="fa fa-upload"></i> Import/Export</button> <button name="saveImport" class="savebutton button"><i class="fa fa-floppy-o"></i> Save</button></div>';
				buf +=
					'<div class="teamedit"><textarea class="textbox" rows="17">' +
					BattleLog.escapeHTML(
						Storage.exportTeam(this.curSetList, this.curTeam.gen)
					) +
					"</textarea></div>";
			} else {
				buf =
					'<div class="pad"><button name="back" class="button"><i class="fa fa-chevron-left"></i> List</button> ';
				buf +=
					'<input class="textbox teamnameedit" type="text" class="teamnameedit" size="30" value="' +
					BattleLog.escapeHTML(this.curTeam.name) +
					'" /> ';
				buf +=
					'<button name="import" class="button"><i class="fa fa-upload"></i> Import/Export</button> ';
				buf += '<div class="teamchartbox">';
				buf += '<ol class="teamchart">';
				buf += "<li>" + this.clipboardHTML() + "</li>";
				var i = 0;
				if (
					this.curSetList.length &&
					!this.curSetList[this.curSetList.length - 1].species
				) {
				...
				if (this.loadingTeam)
					buf +=
						'<div style="message-error">Downloading team from server...</strong><br />';
				buf += '<label name="editMessage" style="display: none">';
				buf +=
					"Remember to click the upload button below to sync your changes to the server!</label><br />";
				if (exports.BattleFormats) {
					buf += '<li class="format-select">';
					buf +=
						'<label class="label">Format:</label><button class="select formatselect teambuilderformatselect" name="format" value="' +
						this.curTeam.format +
						'">' +
						(isGenericFormat(this.curTeam.format)
							? "<em>Select a format</em>"
							: BattleLog.escapeFormat(this.curTeam.format)) +
						"</button>";
					var btnClass =
						"button" +
						(!this.curSetList.length || app.isDisconnected
							? " disabled"
							: "");
					buf +=
						' <button name="validate" class="' +
						btnClass +
						'"><i class="fa fa-check"></i> Validate</button></li>';
				}
				...
				for (i = 0; i < this.curSetList.length; i++) {
					if (
						this.curSetList.length < this.curTeam.capacity &&
						this.deletedSet &&
						i === this.deletedSetLoc
					) {
						buf +=
				...
				if (i === 0) {
					buf +=
						'<li><button name="import" class="button big"><i class="fa fa-upload"></i> Import from text or URL</button></li>';
				}
				if (i < this.curTeam.capacity) {
					buf +=
						'<li><button name="addPokemon" class="button big"><i class="fa fa-plus"></i> Add Pok&eacute;mon</button></li>';
				}
				buf += "</ol>";
				...
				if (formatInfo && formatInfo !== true) {
					if (formatInfo.resources.length || formatInfo.url) {
						buf +=
							'<div style="padding-left: 5px"><h3 style="font-size: 12px">Teambuilding resources for this tier:</h3></div><ul>';
					...
					buf +=
						'<div style="padding-left: 5px">Find ' +
						desc +
						'helpful resources for this tier on <a href="' +
						formatInfo.url +
						'" target="_blank">the Smogon Dex</a>.</div>';
				}
				buf +=
					'<form id="pokepasteForm" style="display:inline" method="post" action="https://pokepast.es/create" target="_blank">';
				...
				buf +=
					'<p><button name="psExport" type="submit" class="button exportbutton"> <i class="fa fa-upload"></i> Upload to Showdown database (saves across devices)</button>';
				var privacy =
					Storage.prefs("uploadprivacy") ||
					typeof Storage.prefs("uploadprivacy") !== "boolean"
						? "checked"
						: "";
				buf +=
					' <label><small>(Private:</small> <input type="checkbox" name="teamprivacy" ' +
					privacy +
					" /><small>)</small></label>";
				buf += "</p>";
				buf +=
					'<p><button name="pokepasteExport" type="submit" class="button exportbutton"><i class="fa fa-upload"></i> Upload to PokePaste</button></p>';
		...
		renderSet: function (set, i) {
			...
			if (!set.species) {
				if (this.deletedSet) {
					buf +=
						'<div class="setmenu setmenu-left"><button name="undeleteSet" class="button"><i class="fa fa-undo"></i> Undo Delete</button></div>';
				}
				buf +=
					'<div class="setmenu"><button name="importSet"><i class="fa fa-upload"></i>Import</button></div>';
				buf +=
					'<div class="setchart" style="background-image:url(' +
					Dex.resourcePrefix +
					'sprites/gen5/0.png);"><div class="setcol setcol-icon"><div class="setcell-sprite"></div><div class="setcell setcell-pokemon"><label>' +
					s_Pokemon +
					'</label><input type="text" name="pokemon" class="textbox chartinput" value="" autocomplete="off" /></div></div></div>';
				buf += "</li>";
				return buf;
			}
			buf += '<div class="setmenu">';
			buf +=
				'<button name="copySet"><i class="fa fa-files-o"></i>Copy</button> <button name="importSet"><i class="fa fa-upload"></i>Import/Export</button> <button name="moveSet"><i class="fa fa-arrows"></i>Move</button> <button name="deleteSet"><i class="fa fa-trash"></i>Delete</button></div>';
			buf += '<div class="setchart-nickname">';
			buf +=
				'<label>Nickname</label><input type="text" name="nickname" class="textbox" value="' +
				BattleLog.escapeHTML(set.name || "") +
				'" placeholder="' +
				BattleLog.escapeHTML(species.baseSpecies) +
				'" />';
			buf += "</div>";
			...
			if (species.cosmeticFormes) {
				buf +=
					'<div class="setcell-sprite changeform"><i class="fa fa-caret-down"></i></div>';
			} else {
				buf += '<div class="setcell-sprite"></div>';
			}
			buf +=
				'<div class="setcell setcell-pokemon"><label>' +
				s_Pokemon +
				'</label><input type="text" name="pokemon" class="textbox chartinput" value="' +
				BattleLog.escapeHTML(set.species) +
				'" autocomplete="off" /></div></div>';

			// details
			buf += '<div class="setcol setcol-details"><div class="setrow">';
			buf +=
				'<div class="setcell setcell-details"><label>Details</label><button class="textbox setdetails" tabindex="-1" name="details">';
			...
			if (renderLevel) {
				buf +=
					'<span class="detailcell detailcell-first"><label>Level</label>' +
					(set.level || 100) +
					"</span>";
			}
			if (this.curTeam.gen > 1) {
				buf +=
					'<span class="detailcell"><label>Gender</label>' +
					GenderChart[set.gender || species.gender || "N"] +
					"</span>";
				if (renderHappiness) {
					buf +=
						'<span class="detailcell"><label>Happiness</label>' +
				...
				if (renderShiny) {
					buf +=
						'<span class="detailcell"><label>Shiny</label>' +
						(set.shiny ? "Yes" : "No") +
						"</span>";
				}
				if (renderHiddenPower) {
					buf +=
						'<span class="detailcell"><label>HP Type</label>' +
						(set.hpType || "Dark") +
						"</span>";
				}
				if (renderDynamax) {
					if (
						...
						set.dynamaxLevel !== 10 &&
						set.dynamaxLevel !== undefined
					) {
						buf +=
							'<span class="detailcell"><label>Dmax Level</label>' +
							(typeof set.dynamaxLevel === "number"
								? set.dynamaxLevel
								: 10) +
							"</span>";
					}
					if (species.canGigantamax || species.forme === "Gmax") {
						buf +=
							'<span class="detailcell"><label>Gmax</label>' +
							(set.gigantamax || species.forme === "Gmax"
								? "Yes"
								: "No") +
							"</span>";
					}
			...
			if (renderItem)
				buf +=
					'<div class="setcell setcell-item"><label>Item</label><input type="text" name="item" class="textbox chartinput" value="' +
					BattleLog.escapeHTML(set.item) +
					'" autocomplete="off" /></div>';
			if (renderAbility)
				buf +=
					'<div class="setcell setcell-ability"><label>Ability</label><input type="text" name="ability" class="textbox chartinput" value="' +
					BattleLog.escapeHTML(set.ability) +
					'" autocomplete="off" /></div>';
			buf += "</div></div>";
			...
			buf +=
				'<div class="setcol setcol-moves"><div class="setcell"><label>Moves</label>';
			buf +=
				'<input type="text" name="move1" class="textbox chartinput" value="' +
				BattleLog.escapeHTML(set.moves[0]) +
				'" autocomplete="off" /></div>';
			buf +=
				'<div class="setcell"><input type="text" name="move2" class="textbox chartinput" value="' +
				BattleLog.escapeHTML(set.moves[1]) +
				'" autocomplete="off" /></div>';
			buf +=
				'<div class="setcell"><input type="text" name="move3" class="textbox chartinput" value="' +
				BattleLog.escapeHTML(set.moves[2]) +
				'" autocomplete="off" /></div>';
			buf +=
				'<div class="setcell"><input type="text" name="move4" class="textbox chartinput" value="' +
				BattleLog.escapeHTML(set.moves[3]) +
				'" autocomplete="off" /></div>';
			buf += "</div>";

			// stats
			buf +=
				'<div class="setcol setcol-stats"><div class="setrow"><label>Stats</label><button class="textbox setstats" name="stats">';
			buf +=
				'<span class="statrow statrow-head"><label></label> <span class="statgraph"></span> <em>' +
				s_EV +
				"</em></span>";
			var stats = {};
			for (var j in BattleStatNames) {
				...
				var s_ev = ev === defaultEV ? "" : ev;
				var evBuf = "<em>" + s_ev + "</em>";
				if (
					BattleNatures[set.nature] &&
					BattleNatures[set.nature].plus === j
				) {
					evBuf += "<small>+</small>";
				} else if (
					BattleNatures[set.nature] &&
					BattleNatures[set.nature].minus === j
				) {
				...
				var statName =
					this.curTeam.gen === 1 && j === "spa"
						? "Spc"
						: BattleStatNames[j];
				buf +=
					'<span class="statrow"><label>' +
					statName +
					'</label> <span class="statgraph"><span style="width:' +
					width +
					"px;background:hsl(" +
					color +
					',40%,75%);"></span></span> ' +
		...
		saveImport: function () {
			...
			if (url) {
				this.$(".teamedit textarea, .teamedit .savebutton").attr(
					"disabled",
					true
				);
				var self = this;
				$.ajax({
					...
					error: function () {
						app.addPopupMessage(
							"Could not fetch a team from this URL. Make sure you copied the full link, or paste the team in by hand."
						);
						self
							.$(".teamedit textarea, .teamedit .savebutton")
							.attr("disabled", null);
		...
		importableUrl: function (value) {
			var match = value.match(
				/^https?:\/\/(pokepast\.es|gist\.github(?:usercontent)?\.com)\/(.*)\s*$/
			);
			if (!match) return;
			...
			switch (host) {
				case "pokepast.es":
					return (
						"https://pokepast.es/" + path.replace(/\/.*/, "") + "/json"
					);
				default: // gist
			...
			if (this.curTeam.dex.modid.includes("monotype")) {
				...
				for (var i = 0; i < this.curSetList.length; i++) {
					...
					if (this.curTeam.gen >= 6) {
						...
						if (
							item.megaStone &&
							species.baseSpecies === item.megaEvolves
						) {
		...
		validate: function () {
			...
			if (
				...
				BattleFormats[format].battleFormat
			) {
		...
		teamNameChange: function (e) {
			...
			if (name.indexOf("[") >= 0 || name.indexOf("]") >= 0) {
				app.addPopupMessage(
					"Names can't contain the characters [ or ], since they're used for storing team IDs."
				);
				name = name.replace(/\[/g, "");
				name = name.replace(/\]/g, "");
			}
		...
		changeFormat: function (format) {
			...
			// since this.curTeam.dex can be initiallized by this.curTeam.format in other places
			...
			this.curTeam.gen = this.curTeam.dex.gen;
			this.save();
			if (this.curTeam.gen === 5 && !Dex.loadedSpriteData["bw"])
		...
		nicknameChange: function (e) {
			...
			this.save();
			if (this.curTeam.dex.modid.includes("crossevolution")) {
				this.update();
				this.updateChart();
			}
		},
		...
		clipboardHTML: function () {
			var buf = "";
			buf +=
				'<div class="teambuilder-clipboard-container" style="display: ' +
				(this.clipboardVisible() ? "block" : "none") +
				';">';
			buf += '<div class="teambuilder-clipboard-title">Clipboard:</div>';
			buf +=
				'<div class="teambuilder-clipboard-data" tabindex="-1">' +
			...
			if (this.curTeam && this.curSetList.length < this.curTeam.capacity) {
				buf +=
					'<button name="pastePokemon" class="teambuilder-clipboard-button-left button"><i class="fa fa-clipboard"></i> Paste!</button>';
			}
			buf +=
				'<button name="clipboardRemoveAll" class="teambuilder-clipboard-button-right button"><i class="fa fa-trash"></i> Clear clipboard</button>';
			buf += "</div>";
			buf += "</div>";
		...
		clipboardInnerHTML: function () {
			...
			for (var i = 0; i < this.clipboardCount(); i++) {
				...
				buf +=
					'<div class="section"><span class="icon" style="' +
					Dex.getPokemonIcon(species.name) +
					'"></span>';
				buf +=
					'<span class="species">' +
					(species.name === species.baseSpecies
						? BattleLog.escapeHTML(species.name)
						: BattleLog.escapeHTML(species.baseSpecies) +
						  "-<small>" +
						  BattleLog.escapeHTML(
								species.name.substr(species.baseSpecies.length + 1)
						  ) +
						  "</small>") +
					"</span></div>";
				buf +=
					'<div class="section"><span class="ability-item">' +
					(BattleLog.escapeHTML(res.ability) || "<i>No ability</i>") +
					"<br />" +
					(BattleLog.escapeHTML(res.item) || "<i>No item</i>") +
					"</span></div>";
				...
				for (var j = 0; j < 4; j++) {
					...
					buf +=
						(BattleLog.escapeHTML(res.moves[j]) || "<i>No move</i>") +
						(!(j & 1) ? "<br />" : "");
					if (j & 1) {
						buf += "</span>";
					}
		...
		clipboardAdd: function (set) {
			...
			if (this.clipboardCount() === 1) {
				var $clipboard = $(".teambuilder-clipboard-container").css(
					"opacity",
					0
				);
		...
		checkPoint: function (i, button) {
			...
			var pointDetails = this.getSetPoint(
				this.curTeam.dex,
				this.curSetList[i]
			);
			app.addPopupMessage(
				"S: " +
					pointDetails[0] +
					" = " +
					pointDetails[1].toFixed(2) +
					" * " +
					pointDetails[2].toFixed(2) +
					" * " +
					pointDetails[3].toFixed(2) +
					" * 10" +
					"\n" +
					"T: " +
					pointDetails[4].toFixed(1) +
					" = " +
					pointDetails[5] +
					(pointDetails[6] === -1 ? " * 1.5" : " + " + pointDetails[6]) +
					"\n" +
					"A: " +
					pointDetails[7] +
					"\n" +
					...
					pointDetails[0] +
					" * " +
					(pointDetails[4] * pointDetails[7] * pointDetails[8]).toFixed(1)
			);
			button.blur();
		},
		...
		getSmogonSets: function () {
			this.$(
				".teambuilder-pokemon-import .teambuilder-import-smogon-sets"
			).empty();
			this.$(
				".teambuilder-pokemon-import .teambuilder-import-user-sets"
			).empty();
		...
		updateCachedUserSets: function (format) {
			...
			for (var i = 0; i < teams.length; i++) {
				...
				for (var j = 0; j < setList.length; j++) {
					var set = setList[j];
					var name =
						set.name + " " + (duplicateNameIndices[set.name] || "");
					var sets = this.userSets[format][set.species] || {};
					sets[name] = set;
					this.userSets[format][set.species] = sets;
					duplicateNameIndices[set.name] =
						1 + (duplicateNameIndices[set.name] || 0);
				}
			}
		},
		...
		importSetButtons: function () {
			...
			if (smogonFormatSets && smogonFormatSets["dex"]) {
				var smogonSets = $.extend(
					{},
					smogonFormatSets["dex"][species],
					(smogonFormatSets["stats"] || {})[species]
				);
				$smogonSetDiv.text("Sample sets: ");
				for (var set in smogonSets) {
					$smogonSetDiv.append(
						'<button name="importSmogonSet" class="button smogon">' +
				...
				$smogonSetDiv.append(
					' <small>(<a target="_blank" href="' +
						this.smogdexLink(species) +
						'">Smogon&nbsp;analysis</a>)</small>'
				);
			}
			...
			if (userFormatSets && userFormatSets[species]) {
				for (var set in userFormatSets[species]) {
					$userSetDiv.append(
						'<button name="importSmogonSet" class="button box">' +
							BattleLog.escapeHTML(set) +
							"</button>"
					);
				}
				...
				$userSetDiv.append(
					"<small>(Sets from your boxes in this format will be available here)</small>"
				);
			}
		},
		importSmogonSet: function (i, button) {
			...
			if (this.$(button).hasClass("smogon")) {
				...
				sampleSet =
					smogonFormatSets["dex"][species][setName] ||
		...
		updateSetView: function () {
			...
			buf +=
				'<button name="back" class="button"><i class="fa fa-chevron-left"></i> Team</button></div>';
			...
			buf +=
				'<div class="pokemonedit-buttons"><button name="closePokemonImport" class="button"><i class="fa fa-chevron-left"></i> Back</button> <button name="savePokemonImport" class="button"><i class="fa fa-floppy-o"></i> Save</button></div>';
		...
		updateSetTop: function () {
			...
			this.$(".teamchart")
				.first()
				.html(this.renderSet(this.curSet, this.curSetLoc));
		},
		renderTeambar: function () {
			...
			if (
				this.curSetList.length &&
				!this.curSetList[this.curSetList.length - 1].species &&
				this.curSetLoc !== this.curSetList.length - 1
			) {
				this.curSetList.splice(this.curSetList.length - 1, 1);
			}
			...
			for (var i = start; i < end; i++) {
				var set = this.curSetList[i];
				var pokemonicon =
					'<span class="picon pokemonicon-' +
					i +
					'" style="' +
					Dex.getPokemonIcon(set) +
					'"></span>';
				if (!set.species) {
					buf +=
						'<button disabled class="addpokemon" aria-label="Add Pok&eacute;mon"><i class="fa fa-plus"></i></button> ';
					isAdd = true;
				} else if (i === this.curSetLoc) {
					buf +=
						'<button disabled class="pokemon">' +
						pokemonicon +
						BattleLog.escapeHTML(
							set.name ||
								this.curTeam.dex.species.get(set.species).baseSpecies ||
								'<i class="fa fa-plus"></i>'
						) +
						"</button> ";
				} else {
					buf +=
						'<button name="selectPokemon" value="' +
						i +
						'" class="pokemon">' +
						pokemonicon +
						BattleLog.escapeHTML(
			...
			if (this.curSetList.length < this.curTeam.capacity && !isAdd) {
				buf +=
					'<button name="addPokemon"><i class="fa fa-plus"></i></button> ';
			}
			return buf;
		},
		updatePokemonSprite: function () {
			...
			this.$(".pokemonicon-" + this.curSetLoc).css(
				"background",
				Dex.getPokemonIcon(set).substr(11)
			);

			var item = this.curTeam.dex.items.get(set.item);
			if (item.id) {
				this.$(".setcol-details .itemicon").css(
					"background",
					Dex.getItemIcon(item).substr(11)
				);
			} else {
		...
		updateStatGraph: function () {
			...
			var defaultEV = this.curTeam.gen > 2 ? 0 : 252;
			var maxEv = 510;
			var renderRemaining = this.curTeam.gen > 2 && supportsEVs;
			var renderRemainingValue = supportsEVs;
			...
			for (var stat in stats) {
				...
				if (
					BattleNatures[set.nature] &&
					BattleNatures[set.nature].plus === stat
				) {
					evBuf += "<small>+</small>";
				} else if (
					BattleNatures[set.nature] &&
					BattleNatures[set.nature].minus === stat
				) {
					evBuf += "<small>&minus;</small>";
				}
				...
				if (color > 360) color = 360;
				var statName =
					this.curTeam.gen === 1 && stat === "spa"
						? "Spc"
						: BattleStatNames[stat];
				buf +=
					'<span class="statrow"><label>' +
					statName +
					'</label> <span class="statgraph"><span style="width:' +
			...
			for (var stat in stats) {
				...
				if (color > 360) color = 360;
				buf +=
					'<div><em><span style="width:' +
					Math.floor(width) +
					"px;background:hsl(" +
					color +
					",85%,45%);border-color:hsl(" +
					color +
					',85%,35%)"></span></em></div>';
			...
			if (pokemonChanged || this.search.qName !== this.curChartName) {
				...
				if (wasIncomplete) {
					if (this.search.find(q)) {
						if (this.search.q)
							this.$chart.find("a").first().addClass("hover");
					}
				}
		...
		updateStatForm: function (setGuessed) {
			...
			if (role === "?") {
				buf +=
					' (Please choose 4 moves to get a guessed spread) (<a target="_blank" href="' +
					this.smogdexLink(species) +
					'">Smogon&nbsp;analysis</a>)</small></p>';
			} else {
				buf +=
					' </small><button name="setStatFormGuesses" class="button">' +
					role +
					": ";
				for (var i in BattleStatNames) {
					if (guessedEVs[i]) {
						var statName =
							this.curTeam.gen === 1 && i === "spa"
								? "Spc"
								: BattleStatNames[i];
						buf += "" + guessedEVs[i] + " " + statName + " / ";
					}
				}
				if (guessedPlus && guessedMinus)
					buf +=
						" (+" +
						BattleStatNames[guessedPlus] +
						", -" +
						BattleStatNames[guessedMinus] +
						")";
				else buf = buf.slice(0, -3);
				buf +=
					'</button><small> (<a target="_blank" href="' +
					this.smogdexLink(species) +
					'">Smogon&nbsp;analysis</a>)</small></p>';
			...
			var evOverride = undefined;
			var maxTotalEVs = 510;
			var s_EV = supportsEVs ? "EV" : "AV";
			var renderRemaining = this.curTeam.gen > 2 && supportsEVs;
			var renderRemainingValue = this.curTeam.gen > 2 && supportsEVs;

			// label column
			buf += '<div class="col labelcol"><div></div>';
			buf +=
				"<div><label>HP</label></div><div><label>Attack</label></div><div><label>Defense</label></div><div>";
			if (this.curTeam.gen === 1) {
				buf += "<label>Special</label></div>";
			} else {
				buf +=
					"<label>Sp. Atk.</label></div><div><label>Sp. Def.</label></div>";
			}
			...
			buf +=
				'<div class="col evcol"><div><strong>' + s_EV + "</strong></div>";
			var totalev = 0;
			this.plus = "";
			this.minus = "";
			for (var i in stats) {
				var val;
				val =
					"" + ((set.evs[i] === undefined ? defaultEV : set.evs[i]) || "");
				if (nature.plus === i) {
					val += "+";
					this.plus = i;
				}
				...
				buf +=
					'<div><input type="text" name="stat-' +
					i +
					'" value="' +
					val +
					'" class="textbox inputform numform" /></div>';
				totalev += set.evs[i] || 0;
			}
			if (renderRemainingValue) {
				if (totalev <= maxTotalEVs) {
					buf +=
						'<div class="totalev"><em>' +
			...
			for (var i in stats) {
				...
				buf +=
					'<div><input type="range" name="evslider-' +
					i +
					'" value="' +
					BattleLog.escapeHTML(
						set.evs[i] === undefined ? "" + defaultEV : "" + set.evs[i]
					...
					'" step="' +
					stepEV +
					'" class="evslider" tabindex="-1" aria-hidden="true" /></div>';
			}
			buf += "</div>";
			...
			if (this.curTeam.gen > 2) {
				...
				if (!set.ivs) set.ivs = {};
				for (var i in stats) {
					if (set.ivs[i] === undefined || isNaN(set.ivs[i]))
						set.ivs[i] = 31;
					var val = "" + set.ivs[i];
					buf +=
						'<div><input type="number" name="iv-' +
						i +
						'" value="' +
						BattleLog.escapeHTML(val) +
						'" class="textbox inputform numform" min="0" max="31" step="1" /></div>';
				}
				var hpType = "";
				if (set.moves) {
				...
				if (hpType && !this.canHyperTrain(set)) {
					...
					switch (hpType) {
						case "dark":
							...
							break;
						case "dragon":
							hpIVs = ["011111", "101111", "110111"];
							break;
						case "ice":
							hpIVs = ["010111", "100111", "111110"];
							break;
						case "psychic":
							hpIVs = ["011110", "101110", "110110"];
							break;
						case "electric":
							hpIVs = ["010110", "100110", "111011"];
							break;
						case "grass":
							hpIVs = ["011011", "101011", "110011"];
							break;
						case "water":
							hpIVs = ["100011", "111010"];
							break;
						...
						case "ghost":
							...
							break;
						case "bug":
							hpIVs = ["100101", "111100", "101100"];
							break;
						case "rock":
							hpIVs = ["001100", "110100", "100100"];
							break;
						case "ground":
							hpIVs = ["000100", "111001", "101001"];
							break;
						case "poison":
							hpIVs = ["001001", "110001", "100001"];
							break;
						case "flying":
							hpIVs = ["000001", "111000", "101000"];
							break;
						case "fighting":
							hpIVs = ["001000", "110000", "100000"];
							break;
					}
					buf +=
						'<div style="margin-left:-80px;text-align:right"><select name="ivspread" class="button">';
					buf +=
						'<option value="" selected>HP ' +
						hpType.charAt(0).toUpperCase() +
						hpType.slice(1) +
						" IVs</option>";
				...
				for (var i in stats) {
					...
					buf +=
						...
						'" value="' +
						BattleLog.escapeHTML(val) +
						'" class="textbox inputform numform" min="0" max="15" step="1" /></div>';
				}
				buf += "</div>";
			}
			...
			if (this.curTeam.gen > 2) {
				buf +=
					'<p style="clear:both">Nature: <select name="nature" class="button">';
				for (var i in BattleNatures) {
					...
					buf +=
						...
						i +
						'"' +
						(curNature === nature ? 'selected="selected"' : "") +
						">" +
						i;
					if (curNature.plus) {
						buf +=
							" (+" +
							BattleStatNames[curNature.plus] +
							", -" +
							BattleStatNames[curNature.minus] +
							")";
					}
					buf += "</option>";
				}
				...
				buf +=
					"<p><small><em>Protip:</em> You can also set natures by typing <kbd>+</kbd> and <kbd>-</kbd> next to a stat.</small></p>";
		...
		checkStatOptimizations: function () {
			...
			if (optimized) {
				...
				for (var i in BattleStatNames) {
					if (optimized.evs[i]) {
						buf +=
							"" + optimized.evs[i] + " " + BattleStatNames[i] + " / ";
					}
				}
				if (!optimized.plus && !optimized.minus) {
					...
					buf +=
						" (+" +
						BattleStatNames[optimized.plus] +
						", -" +
						BattleStatNames[optimized.minus] +
						")";
				}
				buf += "</button>";
		...
		updateNature: function () {
			...
			if (this.plus === "" || this.minus === "") {
				...
				for (var i in BattleNatures) {
					if (
						BattleNatures[i].plus === this.plus &&
						BattleNatures[i].minus === this.minus
					) {
						set.nature = i;
						break;
					}
		...
		statChange: function (e) {
			...
			if (inputName.substr(0, 5) === "stat-") {
				...
				var lastchar = e.currentTarget.value.charAt(
					e.target.value.length - 1
				);
				var firstchar = e.currentTarget.value.charAt(0);
				var natureChange = true;
				if ((lastchar === "+" || firstchar === "+") && stat !== "hp") {
					if (this.plus && this.plus !== stat)
						this.$chart
							.find("input[name=stat-" + this.plus + "]")
							.val(set.evs[this.plus] || "");
					this.plus = stat;
				} else if (
					(lastchar === "-" ||
						lastchar === "\u2212" ||
					...
					if (this.minus && this.minus !== stat)
						this.$chart
							.find("input[name=stat-" + this.minus + "]")
							.val(set.evs[this.minus] || "");
					this.minus = stat;
		...
		statSlide: function (e) {
			...
			var supportsAVs = !supportsEVs; // && this.curTeam.format.endsWith('norestrictions');
			if (supportsEVs) {
				while (val > 0 && this.getStat(stat, set, val - 4) === result)
					val -= 4;
			}
		...
		updateDetailsForm: function () {
			...
			var renderLevel = true;
			var renderHappiness = this.curTeam.gen < 8 || isNatDex;
			var s_Happiness = isLetsGo
				? 'value="70"'
				: 'min="0" max="255" step="1" value="' +
				  (typeof set.happiness === "number" ? set.happiness : 255) +
				  '"';
			var renderShiny = true;
			var renderDynamax = this.curTeam.gen === 8 && !isBDSP;
			var renderWeight = false;
			var renderHiddenPower =
				(this.curTeam.gen < 8 && !isLetsGo) ||
				isNatDex ||
				(isBDSP && species.baseSpecies === "Unown");
			...
			if (renderLevel) {
				buf +=
					'<div class="formrow"><label class="formlabel">Level:</label><div><input type="number" min="1" max="100" step="1" name="level" value="' +
					(typeof set.level === "number" ? set.level : 100) +
					'" class="textbox inputform numform" /></div></div>';
			}

			if (this.curTeam.gen > 1) {
				buf +=
					'<div class="formrow"><label class="formlabel">Gender:</label><div>';
				if (species.gender && !isHackmons) {
					var genderTable = { M: "Male", F: "Female", N: "Genderless" };
					buf += genderTable[species.gender];
				} else {
					buf +=
						'<label class="checkbox inline"><input type="radio" name="gender" value="M"' +
						(set.gender === "M" ? " checked" : "") +
						" /> Male</label> ";
					buf +=
						'<label class="checkbox inline"><input type="radio" name="gender" value="F"' +
						(set.gender === "F" ? " checked" : "") +
						" /> Female</label> ";
					if (!isHackmons) {
						buf +=
							'<label class="checkbox inline"><input type="radio" name="gender" value="N"' +
							(!set.gender ? " checked" : "") +
							" /> Random</label>";
					} else {
						buf +=
							'<label class="checkbox inline"><input type="radio" name="gender" value="N"' +
							(set.gender === "N" ? " checked" : "") +
							" /> Genderless</label>";
					}
				}
				buf += "</div></div>";
				...
				if (renderHappiness) {
					buf +=
						'<div class="formrow"><label class="formlabel">Happiness:</label><div><input type="number" name="happiness" ' +
						s_Happiness +
						' class="textbox inputform numform" /></div></div>';
				}
				...
				if (renderShiny) {
					buf +=
						'<div class="formrow"><label class="formlabel">Shiny:</label><div>';
					buf +=
						'<label class="checkbox inline"><input type="radio" name="shiny" value="yes"' +
						(set.shiny ? " checked" : "") +
						" /> Yes</label> ";
					buf +=
						'<label class="checkbox inline"><input type="radio" name="shiny" value="no"' +
						(!set.shiny ? " checked" : "") +
						" /> No</label>";
					buf += "</div></div>";
				}
				...
				if (renderDynamax) {
					if (!species.cannotDynamax) {
						buf +=
							'<div class="formrow"><label class="formlabel">Dmax Level:</label><div><input type="number" min="0" max="10" step="1" name="dynamaxlevel" value="' +
							(typeof set.dynamaxLevel === "number"
								? set.dynamaxLevel
								: 10) +
							'" class="textbox inputform numform" /></div></div>';
					...
					if (species.canGigantamax || species.forme === "Gmax") {
						buf +=
							'<div class="formrow"><label class="formlabel">Gigantamax:</label><div>';
						if (species.forme === "Gmax") {
							buf += "Yes";
						} else {
							buf +=
								'<label class="checkbox inline"><input type="radio" name="gigantamax" value="yes"' +
								(set.gigantamax ? " checked" : "") +
								" /> Yes</label> ";
							buf +=
								'<label class="checkbox inline"><input type="radio" name="gigantamax" value="no"' +
								(!set.gigantamax ? " checked" : "") +
								" /> No</label>";
						}
						buf += "</div></div>";
				...
				if (renderWeight) {
					buf +=
						'<div class="formrow"><label class="formlabel">Weight:</label><div><input type="number" min="1" max="999" step="1" name="dynamaxlevel" value="' +
						(typeof set.dynamaxLevel === "number"
							? set.dynamaxLevel
							: 10) +
						'" class="textbox inputform numform" /></div></div>';
			...
			if (this.curTeam.gen > 2) {
				buf +=
					'<div class="formrow" style="display:none"><label class="formlabel">Pokeball:</label><div><select name="pokeball" class="button">';
				buf +=
					'<option value=""' +
					(!set.pokeball ? ' selected="selected"' : "") +
					"></option>"; // unset
				...
				for (var i = 0; i < balls.length; i++) {
					buf +=
						...
						balls[i] +
						'"' +
						(set.pokeball === balls[i] ? ' selected="selected"' : "") +
						">" +
						balls[i] +
						"</option>";
				}
			...
			if (renderHiddenPower) {
				buf +=
					'<div class="formrow"><label class="formlabel" title="Hidden Power Type">Hidden Power:</label><div><select name="hptype" class="button">';
				buf +=
					'<option value=""' +
					(!set.hpType ? ' selected="selected"' : "") +
					">(automatic type)</option>"; // unset
			...
			if (renderTeraType) {
				buf +=
					'<div class="formrow"><label class="formlabel" title="Tera Type">Tera Type:</label><div>';
		...
		detailsChange: function (e) {
			...
			// happiness
			var happiness = parseInt(
				this.$chart.find("input[name=happiness]").val(),
				10
			);
			if (isNaN(happiness) || happiness > 255 || happiness < 0)
				happiness = 255;
			...
			// shiny
			var shiny =
				this.$chart.find("input[name=shiny]:checked").val() === "yes";
			if (shiny) {
				set.shiny = true;
			} else {
				delete set.shiny;
			...
			// dynamax level
			var dynamaxLevel = parseInt(
				this.$chart.find("input[name=dynamaxlevel]").val(),
				10
			);
			if (isNaN(dynamaxLevel) || dynamaxLevel > 10 || dynamaxLevel < 0)
				dynamaxLevel = 10;
			...
			// gigantamax
			var gmax =
				this.$chart.find("input[name=gigantamax]:checked").val() === "yes";
			if (gmax) {
				set.gigantamax = true;
			} else {
			...
			if (preEvo) {
				...
				if (preEvoSpecies.exists) {
					if (
						(species.evos || []).includes(preEvoSpecies.name) ||
						(species.forme === "X" &&
							(
								this.curTeam.dex.species.get(species.baseSpecies)
									.evos || []
							).includes(preEvoSpecies.name))
					) {
						set.preEvo = preEvo;
					} else {
						delete set.preEvo;
		...
		chartClick: function (e) {
			...
			if (
				...
				if (moves.length < this.curSet.moves.length) {
					...
					this.$(
						"input[name=move" + Math.min(moves.length + 1, 4) + "]"
					).focus();
		...
		chartKeydown: function (e) {
			var modifier =
				e.shiftKey || e.ctrlKey || e.altKey || e.metaKey || e.cmdKey;
			if (e.keyCode === 13 || (e.keyCode === 9 && !modifier)) {
				...
				if (!$active.length)
					return this.$chart.find("a").first().addClass("hover");
				var $li = $active.parent().prev();
				while ($li[0] && $li[0].firstChild.tagName !== "A")
					$li = $li.prev();
		...
		chartChange: function (e, selectNext) {
			...
			switch (name) {
				...
				case "move4":
					data = dex.species.get(value);
					if (data.exists && modid.endsWith("pokemoves")) {
						val = data.name;
						break;
					}
					data = dex.items.get(value);
					if (
						data.exists &&
						modid.includes("abilityspam") &&
						modid.includes("dualwielding")
					) {
						val = data.name;
						break;
					}
		...
		chartSetCustom: function (val) {
			...
			if (val === "cathy") {
				...
				if (baseFormat.substr(0, 3) === "gen")
					baseFormat = baseFormat.substr(4);
				if (baseFormat.substr(0, 4) === "bdsp")
					baseFormat = baseFormat.substr(4);
				if (baseFormat.substr(0, 8) === "pokebank")
					baseFormat = baseFormat.substr(8);
				if (baseFormat.substr(0, 6) === "natdex")
					baseFormat = baseFormat.substr(6);
				if (baseFormat.substr(0, 11) === "nationaldex")
					baseFormat = baseFormat.substr(11);
				if (baseFormat.substr(-5) === "draft")
					baseFormat = baseFormat.substr(0, baseFormat.length - 5);
				if (!baseFormat) baseFormat = "ou";
				if (this.curTeam && this.curTeam.format) {
					if (
						baseFormat === "battlespotsingles" ||
						baseFormat === "battlespotdoubles" ||
						baseFormat.substr(0, 3) === "vgc" ||
				...
				this.updateSetTop();
				this.$(
					!this.$("input[name=item]").length
						? this.$("input[name=ability]").length
							? "input[name=ability]"
							: "input[name=move1]"
						: "input[name=item]"
				).select();
		...
		unChooseMove: function (moveName) {
			var set = this.curSet;
			if (!moveName || !set || this.curTeam.dex.modid.includes("hiddentype"))
				return;
		...
		canHyperTrain: function (set) {
			if (
				this.curTeam.gen < 7 ||
				this.curTeam.dex.modid.includes("hiddentype")
			)
				return false;
			var format = this.curTeam.format;
			...
			if (
				format.substr(0, 10) === "battlespot" ||
				format.substr(0, 3) === "vgc" ||
				format === "ultrasinnohclassic"
			) {
		...
		chooseMove: function (moveName, resetSpeed) {
			...
			if (this.curTeam.dex.modid.includes("hiddentype")) return;

			var minAtk = true;
			// only available through an event with 31 Atk IVs
			if (
				set.ability === "Battle Bond" ||
				["Koraidon", "Miraidon"].includes(set.species)
			)
				minAtk = false;
			...
			for (var i = 0; i < moves.length; ++i) {
				if (!moves[i]) continue;
				if (moves[i].substr(0, 13) === "Hidden Power ")
					hasHiddenPower = true;
				...
				if (move.id === "transform") {
					...
					!(this.curTeam.gen < 8 && move.id === "rapidspin")
				) {
					minAtk = false;
				} else if (
					[
					...
					(this.curTeam.gen === 5 && move.id === "naturepower")
				) {
					minAtk = false;
				}
		...
		setPokemon: function (val, selectNext) {
			...
			if (this.curTeam && this.curTeam.format) {
				var baseFormat = this.curTeam.format;
				var format =
					window.BattleFormats && window.BattleFormats[baseFormat];
				if (baseFormat.substr(0, 3) === "gen")
					baseFormat = baseFormat.substr(4);
				if (baseFormat.substr(0, 4) === "bdsp")
					baseFormat = baseFormat.substr(4);
				...
				if (this.curTeam && this.curTeam.format) {
					if (
						(baseFormat.substr(0, 10) === "battlespot" &&
							baseFormat.substr(0, 19) !== "battlespotspecial13") ||
						baseFormat.substr(0, 3) === "vgc" ||
						baseFormat.substr(0, 14) === "battlefestival"
					)
						set.level = 50;
					if (baseFormat.startsWith("lc") || baseFormat.endsWith("lc"))
						set.level = 5;
					if (baseFormat.substr(0, 19) === "battlespotspecial17")
						set.level = 1;
			...
			var customMegaStones = {
				...
				"Aerodactyl-Mega-Fantasy": "Aerodactylite",
				...
				"Golisopod-Mega-Fantasy": "Golisopite",
				"Golurk-Mega": "Golurkite",
				"Golurk-Mega-Fantasy": "Golurkite",
			...
			if (
				...
				species.id === "greninjabondfantasy"
			) {
			...
			if (selectNext)
				this.$(
					set.item || !this.$("input[name=item]").length
						? this.$("input[name=ability]").length
							? "input[name=ability]"
							: "input[name=move1]"
						: "input[name=item]"
		...
		getStat: function (stat, set, evOverride, natureOverride) {
			...
			if (stat === "hp") {
				...
				if (!supportsEVs)
					return (
						Math.floor(
							(Math.floor(2 * baseStat + iv + 100) * set.level) / 100 +
								10
						) + (supportsAVs ? ev : 0)
					);
				return Math.floor(
					(Math.floor(2 * baseStat + iv + Math.floor(ev / 4) + 100) *
						set.level) /
						100 +
						10
				);
			...
			var val = Math.floor(
				(Math.floor(2 * baseStat + iv + Math.floor(ev / 4)) * set.level) /
					100 +
					5
			);
			if (!supportsEVs) {
				val = Math.floor(
					(Math.floor(2 * baseStat + iv) * set.level) / 100 + 5
				);
			}
			if (natureOverride) {
				val *= natureOverride;
			...
			if (!supportsEVs) {
				...
				val =
					(Math.floor(val) * friendshipValue) / 100 +
					(supportsAVs ? ev : 0);
			}
		...
		calcBSPoint: function (stats) {
			...
			for (var statName in stats) {
				if (stats[statName] === values_sorted[0])
					stats_adjusted[statName] = values_sorted[1];
			...
			var B = function (h, b) {
				return (2 * h + 200) * (2 * b + 100);
			};
			var S = A;
			var f = function (x) {
				return x * x * x * 11 - x * x * 25 + x * 19 - 0.75;
			};
			var g = function (x) {
				return x * x * x * 10 - x * x * 18 + x * 10 + 2;
			};
			var k = function (x) {
				return -x * x * x * x + x * x * x * 8 - x * x * 10 + x * 3 + 1;
			};
			var A_w = (4 * Math.max(A(a), A(c)) + Math.min(A(a), A(c))) / 1500;
			var B_w =
				(2 * Math.max(B(h, b), B(h, d)) + Math.min(B(h, b), B(h, d))) /
				360000;
			var E = S(s) / 300;
			var f_A_w = f(A_w);
			var g_B_w = g(B_w);
		...
		getSetPoint: function (dex, set) {
			...
			// ability points
			var abilityPoint =
				abilityToPoint[dex.abilities.get(set.ability).id] || 1;
		...
		getTeamPoint: function () {
			...
			for (var i in this.curSetList) {
				var details = this.getSetPoint(
					this.curTeam.dex,
					this.curSetList[i]
				);
				totalPoint += Math.floor(
					details[0] * details[4] * details[7] * details[8]
				);
			}
			return totalPoint;
		},
		...
		initialize: function (data) {
			...
			for (var i = 0; i < data.team.length; i++) {
				...
				if (i !== data.i && i !== data.i + 1) {
					buf +=
						'<li><button name="moveHere" value="' +
						i +
						'" class="option"><i class="fa fa-arrow-right"></i> Move here</button></li>';
				}
				buf +=
					"<li" +
					(i === data.i ? ' style="opacity:.3"' : ' style="opacity:.6"') +
					'><span class="picon" style="display:inline-block;vertical-align:middle;' +
					Dex.getPokemonIcon(set) +
					'"></span> ' +
					BattleLog.escapeHTML(set.name || set.species) +
					"</li>";
			}
			if (i !== data.i && i !== data.i + 1) {
				buf +=
	...
	var DeleteFolderPopup = (this.DeleteFolderPopup = Popup.extend({
		...
		initialize: function (data) {
			...
			var buf =
				'<form><p>Remove "' +
				data.folder.slice(0, -1) +
				'"?</p><p><label><input type="checkbox" name="addname" /> Add "' +
				BattleLog.escapeHTML(this.folder.slice(0, -1)) +
				'" before team names</label></p>';
			buf +=
				'<p><button type="submit"><strong>Remove (keep teams)</strong></button> <!--button name="removeDelete"><strong>Remove (delete teams)</strong></button--> <button type="button" name="close" class="autofocus">Cancel</button></p></form>';
			this.$el.html(buf);
		},
		submit: function (data) {
			this.room.deleteFolder(
				this.folder,
				!!this.$("input[name=addname]")[0].checked
			);
			this.close();
		},
	}));
	var AltFormPopup = (this.AltFormPopup = Popup.extend({
        ...
        initialize: function (data) {
			...
			var buf = "";
			buf +=
				'<p>Pick a variant or <button name="close" class="button">Cancel</button></p>';
		...
		setForm: function (form) {
			...
			if (form && form !== species.form) {
				this.curSet.species = this.curTeam.dex.species.get(
					species.baseSpecies + form
				).name;
			} else if (!form) {
