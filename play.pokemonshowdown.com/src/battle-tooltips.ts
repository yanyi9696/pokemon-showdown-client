...
export class BattleTooltips {
	...
	calculateModifiedStats(clientPokemon: Pokemon | null, serverPokemon: ServerPokemon, statStagesOnly?: boolean) {
		...
		if (clientPokemon) {
			...
			// ==== Aura Burst 状态数值提升补充 ====
            if (clientPokemon.volatiles['auraburstatk']) {
                stats.atk = Math.floor(stats.atk * 1.5);
            } else if (clientPokemon.volatiles['auraburstall']) {
                stats.atk = Math.floor(stats.atk * 1.2);
            }
            
            if (clientPokemon.volatiles['auraburstdef']) {
                stats.def = Math.floor(stats.def * 1.5);
            } else if (clientPokemon.volatiles['auraburstall']) {
                stats.def = Math.floor(stats.def * 1.2);
            }
            
            if (clientPokemon.volatiles['auraburstspa']) {
                stats.spa = Math.floor(stats.spa * 1.5);
            } else if (clientPokemon.volatiles['auraburstall']) {
                stats.spa = Math.floor(stats.spa * 1.2);
            }
            
            if (clientPokemon.volatiles['auraburstspd']) {
                stats.spd = Math.floor(stats.spd * 1.5);
            } else if (clientPokemon.volatiles['auraburstall']) {
                stats.spd = Math.floor(stats.spd * 1.2);
            }
            
            if (clientPokemon.volatiles['auraburstspe']) {
                speedModifiers.push(1.5);
            } else if (clientPokemon.volatiles['auraburstall']) {
	...
	getMoveBasePower(move: Dex.Move, moveType: Dex.TypeName, value: ModifiableValue, target: Pokemon | null = null) {
		...
		if (
			...
			if (move.type === 'Normal') {
				...
				value.abilityModify(this.battle.gen > 6 ? 1.2 : 1.3, "Du Wu Pi Fu");
			}
			if (this.battle.gen > 6) {
	...
	getPokemonTypes(pokemon: Pokemon | ServerPokemon, preterastallized = false): readonly Dex.TypeName[] {
		if (!(pokemon as Pokemon).getTypes) {
			...
			return this.battle.dex.species.getFromPokemon(pokemon).types;
		}
