...
export class PSTeambuilder {
	static packTeam(team: Dex.PokemonSet[]) {
		...
		for (const set of team) {
			...
			if (id === "greninjaashfantasy" || id === "greninjabondfantasy") {
				const abilityId = toID(ability);
				if (!abilityId || abilityId === "moldbreaker") {
					ability = "Chao Yue Qian Ban Bian Shen";
				}
			}
			id = toID(ability);
			buf += `|${id || "-"}`;
			...
			if (set.moves) {
				for (let j = 0; j < set.moves.length; j++) {
					...
					buf += `${j ? "," : ""}${moveid}`;
					if (
						moveid.substr(0, 11) === "hiddenpower" &&
						moveid.length > 11
					) {
			...
			if (set.evs) {
				buf +=
					`|${set.evs["hp"] || ""},${set.evs["atk"] || ""},${
						set.evs["def"] || ""
					},` +
					`${set.evs["spa"] || ""},${set.evs["spd"] || ""},${
						set.evs["spe"] || ""
					}`;
			} else {
				buf += "|";
			...
			if (
				set.pokeball ||
				(set.hpType && toID(set.hpType) !== hasHP) ||
				set.gigantamax ||
		...
		for (const setBuf of buf.split(`]`)) {
			...
			set.ability =
				parts[3] === "-"
					...
					? species.abilities[(parts[3] as "0") || "0"] ||
					  (parts[3] === "" ? "" : "!!!ERROR!!!")
	...
	static exportSet(set: Dex.PokemonSet) {
		...
		if (
			...
			set.dynamaxLevel !== 255 &&
			!isNaN(set.dynamaxLevel)
		) {
	...
	static splitPrefix(
		...
		return [
			buffer.slice(prefixOffset, delimIndex),
			buffer.slice(delimIndex + delimiter.length),
		];
	}
				...
				if (format === ("gen9fantasy" as ID)) {
					...
					if (baseSpeciesData.otherFormes) {
						for (const otherFormeId of baseSpeciesData.otherFormes) {
							...
							if (
								otherFormeData.exists &&
								otherFormeData.forme.startsWith("Mega") &&
								otherFormeData.requiredItems &&
								otherFormeData.requiredItems.length > 0
							) {
								...
								if (
									requiredItemData.exists &&
									(requiredItemData.megaStone ||
										requiredItemData.megaEvolves ===
											baseSpeciesData.name)
								) {
	...
	static importTeamBackup(buffer: string): Team[] {
		...
		for (let line of lines) {
			...
			if (line === "" || line === "---") {
				...
				[curTeam.format, line] = this.splitPrefix(line, "]", 1) as [
					ID,
					string
				];
				if (!curTeam.format) curTeam.format = "gen8" as ID;
				else if (!curTeam.format.startsWith("gen"))
					curTeam.format = `gen6${curTeam.format}` as ID;
...
export function TeamFolder(props: {
	cur?: boolean;
	value: string;
	...
	if (props.cur) {
		return (
			<div class="folder cur">
				<div class="folderhack3">
					<div class="folderhack1"></div>
					<div class="folderhack2"></div>
					<div class="selectFolder" data-value={props.value}>
						{props.children}
					</div>
				</div>
			</div>
...
export function TeamBox(props: {
	...
	if (team) {
		...
		if (!icons) {
			if (!team.packedTeam) {
				...
				icons = PSTeambuilder.packedTeamNames(team.packedTeam).map(
					(species) => (
						<span
							class="picon"
							style={Dex.getPokemonIcon(species)}
						></span>
					)
				);
			}
		...
		contents = [<em>Select a team</em>];
	}
	if (props.button) {
		return (
			<button class="team" value={team ? team.key : ""}>
				{contents}
			</button>
		);
	}
	return (
		<div
			data-href={props.noLink ? "" : `/team-${team ? team.key : ""}`}
			class="team"
			draggable
		>
			{contents}
...
class TeamDropdownPanel extends PSRoomPanel {
	...
	override render() {
		...
		if (!room.parentElem) {
			return (
				<PSPanelWrapper room={room}>
					<p>
						Error: You tried to open a team selector, but you have nothing
						to select a team for.
					</p>
				</PSPanelWrapper>
			);
		}
		// todo: fix
		const baseFormat =
			room.parentElem.getAttribute("data-format") || Dex.currentGen;
		...
		for (const team of teams) {
			const list =
				teamBuckets[team.folder] || (teamBuckets[team.folder] = []);
			list.push(team);
		}
		...
		teamList.push(
			<p>
				{baseFormat.length > 4 && (
					<button
						class={
							"button" + (baseFormat === this.format ? " disabled" : "")
						}
						onClick={this.setFormat}
						name="format"
						value={baseFormat}
					>
						<i class="fa fa-folder-o"></i> [{baseFormat.slice(0, 4)}]{" "}
						{baseFormat.slice(4)}
					</button>
				)}{" "}
				{}
				<button
					...
					value={baseGen}
				>
					<i class="fa fa-folder-o"></i> [{baseGen}]{" "}
					<em>(uncategorized)</em>
				</button>{" "}
				{}
				<button
			...
			teamList.push(
				<p>
					{genList.sort().map((gen) => [
						<button
							class={"button" + (gen === this.gen ? " disabled" : "")}
							onClick={this.setFormat}
							name="gen"
							value={gen}
						>
		...
		for (let folder in teamBuckets) {
			...
			teamList.push(
				<ul class="teamdropdown" onClick={this.click}>
					{teamBuckets[folder].map((team) => (
						<li key={team.key} style={{ display: "inline-block" }}>
		...
		return (
			<PSPanelWrapper room={room} width={width}>
				<div class="pad">
					{teamList}
					{isEmpty && (
						<p>
	...
	override render() {
		...
		if (!room.parentElem) {
			return (
				<PSPanelWrapper room={room}>
					<p>
						Error: You tried to open a format selector, but you have
						nothing to select a format for.
					</p>
				</PSPanelWrapper>
			);
		}
		...
		const selectType: "teambuilder" | "challenge" | "search" =
			(room.parentElem.getAttribute("data-selecttype") as any) ||
			"challenge";

		const formats = Object.values(BattleFormats).filter((format) => {
			if (selectType === "challenge" && format.challengeShow === false)
				return false;
			if (selectType === "search" && format.searchShow === false)
				return false;
			return true;
		});
			...
			<PSPanelWrapper room={room} width={width}>
				<div class="pad">
					...
					{columns.map((column) => (
						<ul class="options" onClick={this.click}>
							{column.map((format) =>
								format.id ? (
									<li>
										<button value={format.name} class="option">
											{format.name
												.replace("[Gen 8 ", "[")
												.replace("[Gen 9] ", "")
												.replace("[Gen 7 ", "[")}
					...
					{noResults && (
						<p>
							<em>
								No formats{!!searchID && ` matching "${searchID}"`}{" "}
								found
							</em>
						</p>
					)}
