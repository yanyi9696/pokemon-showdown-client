...
export const BattleMoveAnims: AnimTable = {
	julengyazuan: {
        anim(scene, [attacker, defender]) {
            ...
            // 3. 触发咬和接触攻击的通用动画 (来自 julengyazuan)
            BattleOtherAnims.bite.anim(scene, [attacker, defender]);
            BattleOtherAnims.contactattack.anim(scene, [attacker, defender]);

            // 4. 钻击特效：气流与冲击波 (来自 drillrun)
            scene.showEffect('wisp', {
                x: defender.x, y: defender.y, z: defender.z,
                scale: 0, opacity: 0.5, time: 300,
            }, {
                scale: 3, opacity: 0, time: 600,
            }, 'linear');
            scene.showEffect('wisp', {
                x: defender.x, y: defender.y, z: defender.z,
                scale: 0, opacity: 0.5, time: 400,
            }, {
                scale: 3, opacity: 0, time: 700,
            }, 'linear');

            scene.showEffect('impact', {
                x: defender.x, y: defender.y, z: defender.z,
                scale: 0, opacity: 0.4, time: 300,
            }, {
                scale: 2, opacity: 0, time: 500,
            }, 'linear');
            scene.showEffect('impact', {
                x: defender.x, y: defender.y, z: defender.z,
                scale: 0, opacity: 0.4, time: 500,
            }, {
                scale: 2, opacity: 0, time: 700,
            }, 'linear');

            // 5. 钻击特效：火花/碎屑飞溅效果 (来自 drillrun)
            scene.showEffect('flareball', {
                x: defender.leftof(10), y: defender.y - 10, z: defender.behind(-40),
                scale: 0.7, yscale: 1, opacity: 0.8, time: 300,
            }, {
                z: defender.behind(5), scale: 0, xscale: 0, yscale: 0, time: 450,
            }, 'accel');
            scene.showEffect('flareball', {
                x: defender.leftof(10), y: defender.y - 10, z: defender.behind(-40),
                scale: 0.7, yscale: 1, opacity: 0.8, time: 350,
            }, {
                z: defender.behind(5), scale: 0, xscale: 0, yscale: 0, time: 500,
            }, 'accel');
            scene.showEffect('flareball', {
                x: defender.leftof(10), y: defender.y - 10, z: defender.behind(-40),
                scale: 0.7, yscale: 1, opacity: 0.8, time: 400,
            }, {
                z: defender.behind(5), scale: 0, xscale: 0, yscale: 0, time: 550,
            }, 'accel');

            // 6. 冰冻结冰特效 (来自 julengyazuan)
            scene.showEffect('icicle', {
                x: defender.x, y: defender.y, z: defender.z,
                scale: 0, opacity: 1, time: 450,
            }, {
                x: defender.x, y: defender.y - 40, z: defender.behind(15),
                scale: 3, opacity: 0, time: 800,
            }, 'linear');
            scene.showEffect('icicle', {
                x: defender.x, y: defender.y, z: defender.z,
                scale: 0, opacity: 1, time: 550,
            }, {
                x: defender.x, y: defender.y - 40, z: defender.behind(15),
                scale: 3, opacity: 0, time: 900,
            }, 'linear');
        },
    },
	leiguanghongming: {
	...
	chaopinyaogunpoyinbo: {
		anim(scene, [attacker, ...defenders]) {
			...
			const defender = defenders[1] || defenders[0];
			scene.backgroundEffect('#6aafd4ff', 300, 0.9);
			scene.backgroundEffect(`url('https://${Config.routes.client}/sprites/gen6bgs/bg-earthycave.jpg')`, 2000, 0.7, 300);
			scene.backgroundEffect('linear-gradient(#FB5C1E 20%, #d9d61dff', 2000, 0.6, 300);
	...
	yuannengshifang: {
		anim(scene, [attacker, defender]) {
			scene.backgroundEffect('#3b214aff', 700, 0.6);
			scene.showEffect('electroball', {
				x: attacker.x,
				y: attacker.y,
				z: attacker.z,
	...
	qingshenglvye: {
		anim(scene, [attacker, defender]) {
			...
			scene.backgroundEffect(`url('https://${Config.routes.client}/fx/weather-grassyterrain.png')`, 750, 1, 800);
		},
	},
	fanchen: {
		anim(scene, [attacker, defender]) {
			...
			scene.backgroundEffect(`url('https://${Config.routes.client}/fx/weather-sandstorm.png')`, 750, 1, 800);
		},
	},
	chabuduowanan: {
		anim(scene, [attacker, defender]) {
	...
	dianshanxunji: {
		anim(scene, [attacker, defender]) {
			...
			// 2. 移除旧特效，换上雷电拳的视觉效果，并调整时间以匹配快速攻击
			scene.showEffect('electroball', {
				x: defender.x,
				y: defender.y,
				z: defender.z,
	...
	yaojingzhiya: {
		anim(scene, [attacker, defender]) {
			scene.backgroundEffect('#dd86c7ff', 450, 0.6, 450);
			BattleOtherAnims.bite.anim(scene, [attacker, defender]);
			attacker.anim({
				x: defender.x,
				y: defender.y + 80,
	...
	mijianbairenchuan: {
		anim(scene, [attacker, defender]) {
			scene.backgroundEffect('#ffffffff', 700, 0.3);
			scene.showEffect('rightslash', {
				x: defender.x + 5,
				y: defender.y + 20,
				z: defender.z,
	...
	chuanyun: {
		anim(scene, [attacker, defender]) {
			scene.backgroundEffect('#c1bfbfff', 700, 0.7);
			scene.showEffect('poisonwisp', {
				x: attacker.x,
				y: attacker.y,
				z: attacker.z,
	...
	huangjinjibanshoulijian: {
		anim(scene, [attacker, defender]) {
			scene.backgroundEffect('linear-gradient(#000000 20%, #f0dd70ff)', 2700, 0.4);
